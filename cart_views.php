...
include 'koneksi1.php';
    ...
    if (isset($_POST["add"])){
        if (isset($_SESSION["cart"])){
            ...
            if (!in_array($_GET["id"],$item_array_id)){
                ...
                echo '<script>window.location="cart_views.php"</script>';
    ...
    if (isset($_GET["action"])){
        if ($_GET["action"] == "delete"){
            ...
            $query = mysqli_query($koneksi, "INSERT INTO transaksi (tgl_transaksi,total_harga) VALUES ('".date("Y-m-d")."','$total')");
            $id_trans = mysqli_insert_id($koneksi);

            foreach($_SESSION["cart"] as $key => $value){
                $id_prod = $value['id_barang'];
                $qty = $value['item_quantity'];
                $sql = "INSERT INTO detail (id_transaksi,kode_kue,qty) VALUES ('$id_trans', '$id_prod', '$qty')";
                ...
                if(!empty($_SESSION["cart"])){
                    ...
                    foreach ($_SESSION["cart"] as $key => $value) {
                        ...
                        <tr>    
                            ...
                            <td>
                                <a href="cart_views.php?action=delete&id=<?php echo $value["id_barang"]; ?>"><span class="text-danger">Hapus</span></a>
                            </td>
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="cart_views.php?action=beli"><span class="text-danger">Beli</span></a>
                            </td>
                        </tr>
                        <?php
                    }
